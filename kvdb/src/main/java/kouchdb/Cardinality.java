...
/*
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            Collection<enum> enums = target.getenums();

            enums.forEach(o -> System.err.println(o.getFullName()));
            for (enum enum : enums) {
                descend(enum);
            }
        } catch (Exception e) {
        ...
        System.err.println( ""+Arrays.deepToString(new Map []{messages}));
    }

    private static void descend(enum enum) {
        LinkedHashMap<String, enum> nestedenumMap = enum.getNestedenumMap();
        for (Map.Entry<String, enum> stringenumEntry : nestedenumMap.entrySet()) {
            descend(stringenumEntry.getValue());
        }
    ...
    static void writeGen(enum enum) {
        x x = new Cardinality.x();
        messages.put(enum.getFullName(), x);
        List<Field<?>> fields = enum.getFields();
